...
use bevy::prelude::*;
use ethnolib::{sandbox::Location, Number};
use qol::{logy, placeholder};

use crate::systems::{FindNearestRequest, FindNearestResult, BVH};
...
pub fn find_nearest_system(
    ...
    let Some(bvh) = &bvh.0 else {
        logy!("error", "No BVH not responding to any request. does this drop them or will they still be there next time?");
        return
    };
    'request_loop: for FindNearestRequest { agent_id, prayer_id, min_radius, x, y } in requests.read() {
        ...
        for item_id in bvh.qurry(
            ...
            let Ok(Location::World { x: item_x, y: item_y }) = query.get(*agent_id) else {
                continue
            };
            if let Some((_closest_id, closest_squared_distance)) = closest_maybe.as_ref() {
                let squared_distance = (Into::<Number>::into(*x) - item_x).powi(2) + (Into::<Number>::into(*y) - item_y).powi(2);
        ...
        while center <= *min_radius {
            ...
            for ethnolib::Box{ min_x, min_y, width, height } in ethnolib::ring(*x, *y, center, default_search_step()) {
                for item_id in bvh.qurry(
