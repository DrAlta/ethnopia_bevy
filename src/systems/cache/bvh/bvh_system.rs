...
use ethnolib::{
    sandbox::{EntityId, Location, world::Size},
    types::AARect,
};

use crate::systems::cache::bvh::BVH;

pub fn bvh_system(query: Query<(Entity, &Location, &Size)>, mut bvh: ResMut<BVH>) {
    ...
    bvh.0 =
        ethnolib::Node::<EntityId>::create_tree(list, &move |id| Some(aa_rects.get(id)?.clone()))
            .ok();
}
