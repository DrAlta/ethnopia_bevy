...
use crate::systems::{cache_inventory::CacheInventory, find_in_inventory::{FindInInventoryRequest, FindInInventoryResult}};
...
pub fn find_in_inventory_system(
    ...
    for FindInInventoryRequest{ agent_id, item_class, action_id } in requests.read(){
        ...
        if let Ok((Some(CacheInventory{ inventory }), _)) = query.get(*agent_id) {
            ...
            let found_item_id_maybe = inventory.iter().find(|x|{
                let Ok((
                    _, 
                    Some(Type(item_type))
                )) = query.get(**x) else {
