...
pub fn picking_system(
    ...
    let who_for: HashMap<Entity, Entity> = item_query
        .iter()
        .map(|(entity, pawn)| (pawn.0.clone(), entity))
        .collect();
    ...
    if let (Some(agent_id), Some(target_id)) = (ui_state.selected_entity, hov) {
        ...
        actions_request_event.send(PosibleActionsRequest {
            agent_id,
            target_id,
        });
    }
...
pub fn hover_out_system(
    ...
    let who_for: HashMap<Entity, Entity> = item_query
        .iter()
        .map(|(entity, pawn, _)| (pawn.0.clone(), entity))
        .collect();
