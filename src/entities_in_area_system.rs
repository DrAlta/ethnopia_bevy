...
pub fn entities_in_area_system(
    ...
    let inquiries: HashMap<Entity, Vec<Aabb2d>> = entities_requests.read().fold(
        HashMap::new(),
        |mut acc,
         EntitiesInAreaRequest {
             agent_id,
             pos,
             size,
         }| {
            let half_size = vec2(size.0 as f32 * 0.5, size.1 as f32 * 0.5);
            let aabb = Aabb2d::new(vec2(pos.0 as f32, pos.1 as f32) + half_size, half_size);
    ...
    let mut reponses: HashMap<Entity, Vec<Vec<Entity>>> = inquiries
        .iter()
        .map(|(k, v)| {
    ...
    for (agent_id, outer) in reponses {
        ...
        for response in outer {
            ...
            entities_in_area_events.send(EntitiesInAreaReponse {
                agent_id,
                entitie_ids: response,
            });
        }
    }
}
